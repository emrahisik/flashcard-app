...
/*
CardOperation comp conditionally renders for either the Add Card page 
or the Edit Card page. "setCardList" prop changes the cardList's
state to re-render the card list on the View Deck page.
*/

const CardOperation = ({deck, isNew, setCardList}) => {
    const { cardId } = useParams();
    const initCard = isNew ? {front:"", back:""} : deck.cards.find(card => card.id===Number(cardId));
    ...
    return (
      <div>
        ...
        <div>
          <h3>{deck.name}: {isNew ? "Add Card" : "Edit Card"}</h3>
        </div>
